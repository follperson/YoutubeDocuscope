...
fviz_nbclust(df_poli_vlog, FUN = hcut, method = "wss")
...
fviz_nbclust(df_poli_vlog, FUN = hcut, method = "silhouette")
...
df_ptgb <- group_by(df_poli_type, Poli.Lean_Type) %>% summarize_all(mean)  %>% filter(Poli.Lean_Type!='Moderate_News Channel')
df_ptgb<- as.data.frame(df_ptgb)
rownames(df_ptgb) <-  df_ptgb$Poli.Lean_Type


#km <- kmeans(df_ptgb[-1], centers = 3, nstart = 25)
km <- kmeans(df_ptgb[-1], centers = 3, nstart = 25)
#km <- kmeans(df_poli_vlog[-1], centers = 3, nstart = 25)
